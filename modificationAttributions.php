...
$rsnbEtabOffrantChambres=$connexion->query("SELECT count(*) as nombreEtabOffrantChambres FROM centre where nombreChambresOffertes!=0");
...
class='tabQuadrille'>";
   ...
   $rsEtab=$connexion->query("SELECT id, nom, nombreChambresOffertes from centre where nombreChambresOffertes!=0 order by id");
   ...
   echo "<td valign='top' width='$pourcCol%'><i>Total de chambres : </i></td>
   </tr>"; 
   ...
   $rsGroupe=$connexion->query("SELECT id, nom, nomPays from equipe where hebergement='O' order by id");
...
echo "<td valign='top' width='$pourcCol%'><i>$totalch</i></td>
   </tr>"; 
      $lgGroupe=$rsGroupe->fetch(PDO::FETCH_ASSOC);  
