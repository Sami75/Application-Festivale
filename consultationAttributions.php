...
if ($nbEtab!=0)
{
   ...
   $req=obtenirReqcentresAyantChambresAttribuées();
   ...
   while($lgEtab!=FALSE)
   {
      ...
      <tr class='ligneTabQuad'>
         <td width='65%' align='left'><i><strong>Nom equipes</strong></i></td>
      ...
      while($lgGroupe!=FALSE)
      {
         ...
         echo "<form method='POST' action='consultationAttributions.php'>
      <input type='hidden' value='validerModifAttrib' name='action'>
       <input type='hidden' value='$idEtab' name='idEtab'>
         <input type='hidden' value='$idGroupe' name='idGroupe'>
            <td width='35%' align='left' style='font-size:20px'>
            <select name='nbChambres'>";
            for ($i=0; $i<=$nbChLib+$nbOccupGroupe; $i++)
            {
               echo "<option";
               if($i==$nbOccupGroupe)
                  echo" selected";
            ...
            echo"</select>&nbsp;&nbsp;&nbsp;<input type='submit' value='Valider' name='valider'></form></td>
         </tr>";
